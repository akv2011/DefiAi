...
interface LeftPanelToggleButtonsProps {
  orientation?: "vertical" | "horizontal";
}

export default function LeftPanelToggleButtons({
  orientation = "vertical",
}: LeftPanelToggleButtonsProps) {
  const { isLeftSidebarExpanded, openLeftSidebar } = useSplitLayout();
    ...
    return (
      <Tooltip>
        ...
        <TooltipContent
          ...
          className="hidden sm:block py-1 px-2 text-xs text-gray-900"
        >
          <p>{label}</p>
        </TooltipContent>
      </Tooltip>
  ...
  return (
    ...
    >
      <TooltipProvider delayDuration={700}>
        ...
        {renderButton(<Plus className="h-4 w-4" />, "New Chat", handleNewChat)}
      </TooltipProvider>
    </div>
  );
}
